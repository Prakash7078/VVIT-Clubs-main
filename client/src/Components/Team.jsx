      ...
      <section className="">
        <div className=" px-4 mx-auto max-w-screen-xl text-center pt-10 ">
          <div className="mx-auto mb-8 max-w-screen-sm ">
            ...
            <p className="font-light sm:text-xl text-brown-400">Coordinators</p>
          </div>
          <div className="md:block  cursor-pointer sm:mx-20 pt-5  ">
            ...
            >
              {data
                .filter(
                  (item) => item?.registeruser?.category === "Coordinator"
                )
                .map((member, index) => {
                  return (
                    <SplideSlide key={index} className="mx-auto md:mx-2">
                      <div className="text-center text-gray-700 w-fit md:mr-5 mx-auto lg:mx-10  ">
                        <img
                          ...
                          src={member?.registeruser?.image}
                          alt="Bonnie Avatar"
                        />
                        <div className="pb-5 shadow-inner shadow-brown-300 bg-white py-3">
                          <h3 className="mb-1 text-2xl font-bold tracking-tight">
                            <p className="font-serif">
                              {member?.registeruser?.username}
                            </p>
                          </h3>
                          <p>{member?.registeruser?.branch}</p>
                          <p>{member?.registeruser?.rollno}</p>
                          <Button
                            color="brown"
                            className="mt-3"
                            onClick={()=>handleDialogData(member?.registeruser?.rollno)}
                          >
                            Profile
                          </Button>
                        </div>
