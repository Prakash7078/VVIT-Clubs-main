...
function AllRegistrations() {
  ...
  const filteredProducts = registers?.filter((product) => {
    if (clubFilter !== "All" && product?.club?.name !== clubFilter) return false;
    if (yearFilter !== "All" && String(product?.registeruser?.year) !== yearFilter)
      return false;
    if (branchFilter !== "All" && product?.registeruser?.branch !== branchFilter) return false;
    if (studentFilter && product?.registeruser?.category !== "Student") return false;
    if (coordinatorFilter && product?.registeruser?.category !== "Coordinator") return false;
    return true;
  });
      ...
      <div className="lg:pt-5 lg:pl-72 pt-5">
        ...
        <div className="flex flex-col lg:flex-row justify-between mb-5 items-center ">
          <div className=" flex items-center mb-3 md:mb-0 flex-col gap-3 ">
            ...
            <div>
              ...
              &nbsp;&nbsp;
              <label
                htmlFor="coordinatorFilter"
                className="lg:ml-4 mr-2 mt-5 lg:mt-0"
              >
                Coordinators
              </label>
              <input
        ...
        <Card className="overflow-x-auto mx-3">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {currentProducts
                ?.filter((item) => item?.registeruser?.rollno === searchroll)
                .map((product) => (
                  <tr
                    key={product?.registeruser?._id}
                    className="bg-brown-200 font-bold"
                  >
                    <td className="border-none text-center px-4 py-2">
                      <Link to={`${product?.registeruser?.rollno}/profile`}>
                        <img
                          className="rounded-full w-10 h-10 mx-auto"
                          src={product?.registeruser?.image}
                        />
                      </Link>
                    </td>
                    <td className="border-none text-center px-4 py-2">
                      {product?.registeruser?.username}
                    </td>
                    <td className="border-none text-center px-4 py-2">
                      {product?.club?.name}
                    </td>
                    <td className="border-none text-center px-4 py-2">
                      {product?.registeruser?.category}
                    </td>
                    <td className="border-none text-center px-4 py-2">
                      {product?.registeruser?.year}
                    </td>
                    <td className="border-none text-center px-4 py-2">
                      {product?.registeruser?.branch}
                    </td>
                    <td className="border-none text-center px-4 py-2">
                      {product?.registeruser?.rollno}
                    </td>
                    ...
                    <td className="border text-center px-4 py-2 ">
                      <AiOutlineDelete
                        ...
                        size={20}
                        onClick={() =>
                          handleDelete(product?.registeruser?.rollno)
                        }
              ...
              {currentProducts?.map((product) => (
                <tr key={product?.registeruser?._id}>
                  <td className="border text-center px-4 py-2">
                    <Link to={`${product?.registeruser?.rollno}/profile`}>
                      <img
                        className="rounded-full w-10 h-10 mx-auto"
                  ...
                  <td className="border text-center px-4 py-2">
                    <div className="w-fit mx-auto cursor-pointer">
                      {product?.registeruser?.category === "Student" ? (
                        <BsFillPersonPlusFill
                          size={20}
                          onClick={() =>
                            handleUpdate(product?.registeruser?.category, product?.registeruser?.rollno)
                          }
                        />
                      ) : (
                        <BsFillPersonDashFill
                  ...
                  <td className="border text-center px-4 py-2 ">
                    <AiOutlineDelete
                      color="red"
                      size={20}
                      onClick={() => handleDelete(product?.registeruser?.rollno)}
                      className="cursor-pointer"
                    />
                  </td>
                </tr>
